...
class SafeTrajClient
{
 public:
  ...
  ///If the server is in the "motionError" state, waits until motion stops, 
  ///recalibrates the emulator, and resets the error flag.  Returns true if
  ///the status had to be reset.
  bool CheckAndResetErrors();
  ...
  std::string Status() { return (virtualController?mirror.Status():t.Status()); }
  bool ClearError() { return (virtualController?mirror.ClearError():t.ClearError()); }
  ...
  MotionResult Brake(double tSystem,double dt);

  ///Adds a braking trajectory starting at absolute time t
  MotionResult BrakeAbs(double t);
   ...
   */
  MotionResult ResetTrajectoryAbs(double t);
   ...
   * followed by addMilestone commands that slow the robot gradually.
   */
  MotionResult ResetTrajectoryRel(double dt);
   ...
   */
  MotionResult MoveTo(const Vector& q,double rate=1.0,double dt=0.0);
   ...
   * maximum speed and acceleration is multiplied by rate.
   */
  MotionResult MoveToImmediate(double tSystem,double dt,const Vector& q,double rate=1.0);
   ...
   */
  MotionResult MoveTraj(const std::vector<Vector>& q,const std::vector<Vector>& dq,const std::vector<double>& dt,double rate=1.0);
  MotionResult MoveTraj(const std::vector<Vector>& q,const std::vector<Vector>& dq,double rate=1.0);
  MotionResult MoveTraj(const std::vector<Vector>& q,const std::vector<double>& dt,double rate=1.0);
  MotionResult MoveTraj(const std::vector<Vector>& q,double rate=1.0);
  ...
  MotionResult AddTrajectory(TrajectoryFunction f,double tmax);
  MotionResult AddTrajectory(const Spline::PiecewisePolynomialND& f);
  MotionResult AddTrajectory(const std::vector<double>& t,const std::vector<Vector>& q,const std::vector<Vector>& v);
   ...
   */
  template <class TrajectoryFunction>
  MotionResult AddTrajectoryImmediate(double tstart,TrajectoryFunction f,double tmax);
  MotionResult AddTrajectoryImmediate(double tstart,const Spline::PiecewisePolynomialND& f);
  MotionResult AddTrajectoryImmediate(double tstart,const std::vector<double>& t,const std::vector<Vector>& q,const std::vector<Vector>& v);
  ...
  /// Set tstart < 0 to check from the end of the motion queue.
  /// Otherwise, this checks for a splice away form the current queue
  template <class TrajectoryFunction>
  bool CheckTrajectory(double tstart,TrajectoryFunction f,double tmax);
  bool CheckTrajectory(double tstart,const Spline::PiecewisePolynomialND& f);
  bool CheckTrajectory(double tstart,const std::vector<double>& t,const std::vector<Vector>& q,const std::vector<Vector>& v);
 ...
 private:
   ...
   */
  template <class TrajectoryFunction>
  MotionResult SendTrajectory(double tstart,TrajectoryFunction f,double tmax,double dt=0.0);
  MotionResult SendTrajectory(double tstart,const Spline::PiecewisePolynomialND& f);
  MotionResult SendTrajectory(double tstart,const std::vector<double>& t,const std::vector<Vector>& q,const std::vector<Vector>& v);
  ...
  std::vector<Vector> sendvs;
  std::vector<double> sendts;
  int sendIndex;
};

template <class TrajectoryFunction>
SafeTrajClient::MotionResult SafeTrajClient::AddTrajectory(TrajectoryFunction f,double tmax)
{
  if(!this->CheckTrajectory(-1,f,tmax)) return FailedCheck;
  return this->SendTrajectory(-1,f,tmax);
}

template <class TrajectoryFunction>
SafeTrajClient::MotionResult SafeTrajClient::AddTrajectoryImmediate(double tstart,TrajectoryFunction f,double tmax)
{
  if(!this->CheckTrajectory(tstart,f,tmax)) return FailedCheck;
...
bool SafeTrajClient::CheckTrajectory(double tstart,TrajectoryFunction f,double tmax) 
{
  ...
  if(tstart < 0) {  //add to end of motion queue
    fprintf(stderr,"Non-absolute start time deprecated\n");
    abort();
    GetEndConfig(qprev);
...
template <class TrajectoryFunction>
SafeTrajClient::MotionResult SafeTrajClient::SendTrajectory(double tstart,TrajectoryFunction f,double tmax,double dt)
{
  //250Hz
  ...
  std::vector<double> ts((int)ceil(tmax/dt));
  std::vector<Vector> qs(ts.size());
  std::vector<Vector> vs(ts.size());
  if(!ts.empty()) {
    double t=std::max(tstart,0.0);
    ...
    vs[0].resize(qs[0].size(),0.0);
    vs.back() = vs[0]; //zero end velocity
    for(size_t i=1;i+1<qs.size();i++) {
      vs[i].resize(qs[i+1].size());
      for(size_t j=0;j<vs[i].size();j++)
	vs[i][j] = (qs[i+1][j]-qs[i-1][j])/(ts[i+1]-ts[i-1]);
    }
    if(tstart >= 0) mirror.GetTrajVelocity(tstart,vs.front());
  ...
  sendvs.insert(sendvs.end(),vs.begin(),vs.end());
