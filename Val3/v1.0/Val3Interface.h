...
/** A class that interfaces between RobotSim and a TrajServer-based device. 
 * The configuration is specified in an XML file given to the Load
 * method, which contains a mapping from RobotSim to/from TrajServer,
 * as well as optional initialization commands.
 *
 * The user connects with the network location of the device and the version
 * of TrajServer that is used.  Currently only TrajServer version 1.0 is
 * supported.
 *
...
class Val3Interface
{
 public:
  ...
  ///Calling thread calls this periodically
  void SendPoll();

  ///Appends a smooth move to a new configuration from the end of the motion
  ///queue
  bool MoveTo(const Config& q,double rate=1.0,double dt=0.0);
  ...
  bool MoveToImmediate(double dt,const Config& q,double rate=1.0);
  ...
  bool MoveRamp(const ParabolicRamp::ParabolicRampND& ramp);
  ...
  ///at zero velocity.
  bool MoveRampImmediate(double tstart,const ParabolicRamp::ParabolicRampND& ramp);
  ...
  bool MoveRamps(const std::vector<ParabolicRamp::ParabolicRampND>& ramp);
  ...
  ///end at zero velocity.
  bool MoveRampsImmediate(double tstart,const std::vector<ParabolicRamp::ParabolicRampND>& ramp);
