...
void ToTrajClient(const ParabolicRamp::ParabolicRampND& ramp,ParabolicRamp::ParabolicRampND& castRamp,const Val3Interface* iface)
{
...
bool Val3Interface::LoadSetup(const char* setupXml)
{
  Disconnect();
  TiXmlDocument doc;
...
bool Val3Interface::Connect(const char* host,int port,double version)
{
  if(version != 1.0) { 
    fprintf(stderr,"Val3Interface: only version 1.0 is currently supported\n");
    return false;
  }
  Disconnect();
...
bool Val3Interface::ConnectInternalEmulator(const char* specFile)
{
  Disconnect();
...
void Val3Interface::Disconnect()
{
  client.Disconnect();
}

void Val3Interface::SendPoll()
{
...
void Val3Interface::ConfigToTrajClient(const Config& q,TrajClient::Vector& v) const
{
...
void Val3Interface::ConfigFromTrajClient(const TrajClient::Vector& v,Config& q) const
{
...
void Val3Interface::VectorToTrajClient(const Vector& q,TrajClient::Vector& v) const
{
  v.resize(robotFromModel.size());
  for(size_t i=0;i<robotFromModel.size();i++) {
    double scale=robotFromModel[i].scale;
...
void Val3Interface::VectorFromTrajClient(const TrajClient::Vector& v,Vector& q) const
{
  q.resize(modelFromRobot.size());
  for(size_t i=0;i<modelFromRobot.size();i++) {
    double scale=modelFromRobot[i].scale;
...
void Val3Interface::UpdateModelCurrent(Robot* model)
{
  GetConfig(model->q);
...
bool Val3Interface::MoveTo(const Config& q,double rate,double dt)
{
  TrajClient::Vector v;
...
bool Val3Interface::MoveToImmediate(double dt,const Config& q,double rate)
{
  TrajClient::Vector v;
  ConfigToTrajClient(q,v);
...
bool Val3Interface::MoveRamp(const ParabolicRamp::ParabolicRampND& ramp)
{
  return MoveRampImmediate(0,ramp);
}

bool Val3Interface::MoveRampImmediate(double tstart,const ParabolicRamp::ParabolicRampND& ramp)
{
  ParabolicRamp::ParabolicRampND castRamp;
  ToTrajClient(ramp,castRamp,this);
  return client.AddTrajectoryImmediate(tstart,Cast(castRamp),ramp.endTime);
}

bool Val3Interface::MoveRamps(const std::vector<ParabolicRamp::ParabolicRampND>& ramps)
{
  return MoveRampsImmediate(0,ramps);
}

bool Val3Interface::MoveRampsImmediate(double tstart,const std::vector<ParabolicRamp::ParabolicRampND>& ramps)
{
  if(ramps.empty()) {
    if(tstart > 0) {
  ...
  return client.AddTrajectoryImmediate(tstart,poly,poly.EndTime());
}


///basic queries
bool Val3Interface::GetConfig(Config& q)
{
  TrajClient::Vector x;
...
bool Val3Interface::GetVelocity(Vector& v)
{
  TrajClient::Vector x;
...
bool Val3Interface::GetTransform(RigidTransform& T)
{
  TrajClient::Vector x;
...
bool Val3Interface::GetJointLimits(Config& jmin,Config& jmax)
{
...
bool Val3Interface::GetVelocityLimits(Vector& vmax)
{
  TrajClient::Vector x;
...
bool Val3Interface::GetAccelerationLimits(Vector& amax)
{
  TrajClient::Vector x;
...
bool Val3Interface::GetDecelerationLimits(Vector& dmax)
{
  TrajClient::Vector x;
...
bool Val3Interface::GetEndConfig(Config& qend)
{
  TrajClient::Vector x;
...
bool Val3Interface::GetEndVelocity(Vector& vend)
{
  TrajClient::Vector x;
...
void Val3Interface::SetJointLimits(const Vector& qmin,const Vector& qmax)
{
  TrajClient::Vector xmin,xmax;
...
void Val3Interface::SetSpeedScale(double rate)
{
